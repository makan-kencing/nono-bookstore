    ...
    #[\Override] public function createQuery(PDO $pdo): PDOStatement
    {
        $stmt = $pdo->prepare('
            SELECT b.id                `id`,
                   ...
                   ru.id               `ratings.user.id`,
                   ru.username         `ratings.user.username`,
                   ru.email            `ratings.user.email`,
                   ru.hashed_password  `ratings.user.hashedPassword`,
                   ru.role             `ratings.user.role`,
                   ru.is_verified      `ratings.user.isVerified`,
                   rr.id               `ratings.replies.id`,
                   rr.content          `ratings.replies.content`,
                   rru.id              `ratings.replies.user.id`,
                   rru.username        `ratings.replies.user.username`,
                   rru.email           `ratings.replies.user.email`,
                   rru.hashed_password `ratings.replies.user.hashedPassword`,
                   rru.role            `ratings.replies.user.role`,
                   rru.is_verified     `ratings.replies.user.isVerified`
            FROM book b
